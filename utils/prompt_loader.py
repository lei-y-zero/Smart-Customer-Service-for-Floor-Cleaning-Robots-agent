...
def load_rag_prompts():
    ...
    try:
        ...
        logger.error(f"[load_rag_prompts]解析RAG总结词出错，{str(e)}")
        raise e

def load_report_prompts():
    try:
        rag_report_path=get_abs_path(prompts_conf["report_prompt_path"])
    except KeyError as e:
        logger.error(f"[load_report_prompts]在yaml配置项中没有report_prompt_path配置项")
        raise e

    try:
        return open(rag_report_path,"r",encoding='utf-8').read()
    except Exception as e:
